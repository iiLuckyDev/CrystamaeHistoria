...
extends LimitedUseItem {
    ...
    private static final Map<Material, Material> CONVERSIONS = new EnumMap<Material, Material>(Material.class);
    private static final Map<EntityType, EntityType> ENTITY_CONVERSIONS = new EnumMap<EntityType, EntityType>(EntityType.class);
    ...
    public ItemUseHandler getItemHandler() {
        return e -> {
            ...
            Optional<Entity> entityOptional = Optional.ofNullable(player.getTargetEntity(10, false));
            e.cancel();
            blockOptional.ifPresent(block -> this.convertBlock(player, (Block)block));
            entityOptional.ifPresent(entity -> this.entities(player, (Entity)entity));
            this.damageItem(player, e.getItem());
        };
    }
    ...
    private void convertBlock(@Nonnull Player player, @Nonnull Block block) {
        if (GeneralUtils.hasPermission(player, block, Interaction.BREAK_BLOCK)) {
            Displacer.convertBlock(block);
        }
    }

    private void entities(@Nonnull Player player, @Nonnull Entity entity) {
        if (!GeneralUtils.hasPermission(player, entity.getLocation(), Interaction.INTERACT_ENTITY) || !GeneralUtils.hasPermission(player, entity.getLocation(), Interaction.ATTACK_ENTITY)) {
            return;
        }
        EntityType entityType = entity.getType();
        EntityType convertTo = ENTITY_CONVERSIONS.get(entityType);
        if (convertTo != null) {
            ...
            this.itemFrame((ItemFrame)entity);
        } else if (entityType == EntityType.ZOMBIE_VILLAGER) {
            this.zombieVillager(player, (ZombieVillager)entity);
        }
    }
    ...
    public static void convertBlock(@Nonnull Block block) {
        if (BlockStorage.check((Block)block) != null) {
            return;
        }
        ...
        if (convertTo != null) {
            ...
            Displacer.farmland(block);
        } else if (material == Material.DIRT) {
            Displacer.dirt(block);
        }
    }
