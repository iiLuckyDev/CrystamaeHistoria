...
import io.github.sefiraat.crystamaehistoria.utils.Util;
...
public enum Skulls {
    ...
    private static final Skulls[] cachedValues;
    private final String hash;
    @Nullable
    private final StoryType storyType;

    @ParametersAreNonnullByDefault
    private Skulls(String hash, StoryType storyType) {
        this.hash = hash;
    ...
    public static Skulls getByType(StoryType type) {
        for (Skulls skull : Skulls.values()) {
            if (skull.storyType != type) continue;
            return skull;
        }
        return null;
    }
    ...
    public ItemStack getPlayerHead() {
        return Util.fromBase64Hash(this.hash);
    }
